...
export class GameState {
  ...
  addPlayer(uid: number, name: string): void {
    if (this.player.uid == null) {
      this.player.uid = uid;
  ...
  setPlayerLevel(uid: number, level: number): void {
    if (this.isMe(uid)) this.player.level = level;
  }

  trackPlayerXp(exp: number): void {
    this.playerXp.track(exp);
  }

  trackMercXp(exp: number): void {
    this.mercXp.track(exp);
  }

  move(uid: number, x: number, y: number): void {
    if (this.isMe(uid)) {
  ...
  kills(uid: number, count: number): void {
