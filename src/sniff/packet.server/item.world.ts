...
export class GSPacketItemWorldAction extends GamePacket {
  ...
  static isOnGround(action: ItemActionType): boolean {
    return (
      action === ItemActionType.AddToGround ||
  ...
  trackCode(): number {
    ...
    if (TrackCode[this.code]) return Log.WARN;

    if (this.code === 'cx5' || this.code === 'cx8') return Log.WARN;
    if (this.code.match(/(r[0-9][0-9])/)) return Log.WARN;
    if (this.quality === ItemQuality.Set) return Log.WARN;
    if (this.quality === ItemQuality.Unique) return Log.WARN;

    if (this.code === 'gld') return 0;
    if (this.quality === ItemQuality.NotApplicable) return 0;
    if (this.sockets === 1) return 0;
    if (this.quality === ItemQuality.Normal) return 0;
    return Log.DEBUG;
  }
